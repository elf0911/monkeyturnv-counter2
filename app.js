...
const GDEF={
 ...
 ticket:{label:"舟券",children:[["ticketDefault","通常"],["ticketHigh","銀（偶数濃厚）"],["ticketOver4","金（設定4以上）"],["ticketOver5","設定5以上"],["ticketSix","虹（設定6）"]]}
};
...
for(const [k,,fixed] of DISPLAY){DEF.visible[k]=["games","normalGames","atGames","atHit"].includes(k); DEF.judgeUse[k]=k==="games"||k==="atHit";}
...
if(S.visible.games&&S.judgeUse.games){ /* 総ゲーム数は各分母として使用。単独では期待度を動かさない */ }
...
if(S.data.trophyBronze)l.push("トロフィー 銅（設定2以上濃厚）");
if(S.data.trophyGold)l.push("トロフィー 金（設定4以上濃厚）");
if(S.data.trophyKerot)l.push("トロフィー ケロット柄（設定5以上濃厚）");
if(S.data.ticketHigh)l.push("舟券 銀（偶数設定濃厚）");
if(S.data.ticketOver4)l.push("舟券 金（設定4以上濃厚）");
...
function jitems(){let keys=["games","atHit","fiveCoin",...GROUPS].filter(k=>S.visible[k]);$("judgeItems").innerHTML=keys.map(jitem).join("");$("judgeItems").querySelectorAll("[data-use]").forEach(c=>c.onchange=()=>{S.judgeUse[c.dataset.use]=c.checked;save();render()});$("judgeItems").querySelectorAll("[data-open]").forEach(el=>el.onclick=e=>{if(e.target.tagName==="INPUT")return;el.parentElement.classList.toggle("open")})}
function jitem(k){let checked=S.judgeUse[k]?"checked":"",title,main="-",near="",pub="";if(k==="games"){title="総ゲーム数";main=`${S.data.games||0}G`;near="判別の分母";pub="AT初当たり・5枚役などの分母として使用します。"}else if(k==="atHit"||k==="fiveCoin"){let inf=PUB[k],c=S.data[k]||0,den=(k==="fiveCoin"?S.data[S.fiveCoinBase]:S.data.normalGames)||0,rn=c&&den?den/c:null;title=inf.label;main=rn?`1/${trim(rn,1)}`:"-";near=rn?`（${nearest(rn,inf.values)}）`:"（-）";pub=Object.entries(inf.values).map(([s,v])=>`設定${s}　1/${v}`).join("<br>")}else{title=GDEF[k].label;main=gtotal(k)?`${gtotal(k)}回`:"-";pub=gtext(k)}let imp=S.showImpact?`<div class="starImpact">影響度 ${impactStars(k)}</div>`:"";return `<div class="jitem"><div class="jsum" data-open="${k}"><input type="checkbox" data-use="${k}" ${checked}><div><div class="jtitle">${title}</div><div class="jrate">${main}</div><div class="near">${near}</div>${imp}</div><div class="chev">▼</div></div><div class="pub">${pub}</div></div>`}
function gtext(g){
...
function pois(sc,count,games,vals,weight){
  if(!count||!games)return;
  for(const set of SETS){
    const p=1/(vals[set]||999999);
    const lambda=Math.max(games*p,0.000001);
    const logLike=count*Math.log(lambda)-lambda;
    sc[set]*=Math.exp(logLike*weight);
  }
}
function allowed(){
  let a=[...SETS];
  const keep=arr=>{a=a.filter(s=>arr.includes(s))};
  if(S.data.trophyRainbow||S.data.ticketSix||S.data.medalRainbow||S.data.endOmedeto)keep([6]);
  if(S.data.trophyKerot||S.data.ticketOver5)keep([5,6]);
  if(S.data.trophyGold||S.data.ticketOver4||S.data.medalRed||S.data.voiceTeio||S.data.directWeak||S.data.directWeakChance)keep([4,5,6]);
  if(S.data.trophyBronze)keep([2,4,5,6]);
  if(S.data.ticketHigh||S.data.voiceOtsukare)keep([2,4,6]);
  return a.length?a:SETS;
}
function impactStars(k){
  const n={games:1,atHit:3,fiveCoin:4,direct:5,rival:3,chargeItem:3,chargeVoice:3,endingVoice:2,rare:2,medal:3,trophy:5,ticket:5}[k]||2;
